...
namespace RMS.BLL
{
    public class ProductBiz
    {
        ...
        // Display all the product details in gridview on Stock page

        public DataTable GetProductStockList(string searchText, decimal stockQty)
        {
            objDataAccess = DataAccess.NewDataAccess();
            objDbCommand = objDataAccess.GetCommand(true, IsolationLevel.ReadCommitted);
            //DbDataReader objDbDataReader = null;
            ...
            {
                objDbCommand.AddInParameter("SearchText", searchText);
                objDbCommand.AddInParameter("StockQty", stockQty);
                dt = objDataAccess.ExecuteTable(objDbCommand, "[rdb].[uspGetProductInfoByStockQtyForGv]", CommandType.StoredProcedure);
            }
            catch (Exception ex)
            {
                throw new Exception("Error : " + ex.Message);
